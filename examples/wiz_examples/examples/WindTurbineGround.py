...
# --- Local
from floris.simulation.wake_vortex.WindTurbine import WindTurbine
...
def main():
    ...
    for ye in YE:
        ...
        nz=50
        ny=51
        y = np.linspace(-4*R,2*R,ny)+h_hub
        ...
        im=ax.contourf(Z/R,Y/R,Speed,levels=10)#,levels=30,vmin=0,vmax=1.0)
        Rotor=WT.rotor_disk_points()
        ax.plot(Rotor[2,:]/R,Rotor[0,:]/R,'k--')
        cb=fig.colorbar(im)
        sp=ax.streamplot(z/R,y/R,uz.T,uy.T,color='k',linewidth=0.7,density=2)
        ax.set_xlabel('y/R [-]')
        ax.set_ylabel('x/R [-]')
        deg=180/np.pi
