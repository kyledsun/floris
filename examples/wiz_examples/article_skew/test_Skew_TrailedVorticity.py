...
# --- Local
from floris.simulation.wake_vortex.VortexCylinderSkewed import fV_Trailed
try:
...
def main(test=False):
    ...
    plt.ylabel('Vertical induced velocity $u_{y,l} / u_{y,0}$')
    ...
    plt.legend()
    plt.xlabel('Azimuthal position $\psi$ [deg]')
    plt.ylabel('Axial induced velocity $u_{z,l} / u_{y,0}$')
    plt.xlim(np.array([- 180,180]))
    # set(gca,'XTick',np.array([- 180,- 90,0,90,180]))
