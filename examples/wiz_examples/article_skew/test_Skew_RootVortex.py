...
# --- Local
from floris.simulation.wake_vortex.VortexCylinderSkewed import fV_Root
...
def main(test=False):
    ...
    plt.plot(vpsi*180/pi, np.squeeze(v_z_root0[:,0,:])*4*pi*R/Gamma_tot,'k--',linewidth=2,label='Value for $\chi=0$')
    plt.legend()
    plt.ylabel('$u_{z,r} 4 \pi R / \Gamma_{tot}$')
    plt.xlabel('Azimuthal position $\psi$ [deg]')
    plt.title('RootVortexOnly Axial velocity for various radii Chi%2d'%np.round(chi * 180 / pi))
    plt.xlim(np.array([0,360]))

    plt.figure()
    for ir,r in enumerate(vr):
        plt.plot(vpsi*180/pi,np.squeeze(v_psi_root[:,ir,:])*4*pi*R/Gamma_tot, label='$r/R=%2.1f$'%r)
    plt.plot(vpsi*180/pi, np.squeeze(v_psi_root0[:,0,:])*4*pi*R/Gamma_tot,'k--',linewidth=2,label='Value for $\chi=0$')
    plt.legend()
    plt.ylabel('$u_{\psi,r} 4 \pi R / \Gamma_{tot}$')
    plt.xlabel('Azimuthal position \psi [deg]')
    plt.title('RootVortexOnlyTangential velocity for various radii Chi%2d'%np.round(chi*180/pi))
