...
# --- Local
from floris.simulation.wake_vortex.VortexCylinderSkewed import fV_Tangential, fKxit, fKzt
try:
...
def main(test=False):
    ...
    scale = 1
    LW = 1
    plt.quiver(X,Y - uz.ravel() / scale,0*X,uz.ravel()/scale, scale=1,angles='xy',scale_units='xy',headwidth=2,headlength=3,headaxislength=3)
    # Restoring Moment
    # ArrowLength = 0.22
    # ArrowAngle = 30
    # MomentStart = 0.3
    ...
    plt.ylabel('Axial induced velocity $-u_{z,t}/\gamma_t$ ')
    ...
    plt.legend(legds)
    plt.xlabel('Radial position $r/R$')
    plt.ylabel('Flow expansion $F_{t} (r,\chi)$')
